...
# Pentatonic root frequencies for each group (up to 5 groups)
# Spaced enough apart to be instantly distinguishable by ear
GROUP_ROOT_FREQS = [440, 554, 659, 784, 932]   # A4 C#5 E5 G5 A#5

# Interval multipliers for each member slot within a group
# Lead (0) = root, joiner 1 = fifth up, joiner 2 = major third up,
# joiner 3 = octave up, joiner 4 = minor seventh
MEMBER_FREQ_MULT = [1.0, 1.5, 1.25, 2.0, 1.778]

def _tone_key(gid: int, vid: int, grp_idx: int, member_idx: int) -> str:
    return f"g{gid}_vc{vid}_tone_g{grp_idx}_m{member_idx}"

def _tone_freq(grp_idx: int, member_idx: int) -> float:
    root = GROUP_ROOT_FREQS[grp_idx % len(GROUP_ROOT_FREQS)]
    mult = MEMBER_FREQ_MULT[member_idx % len(MEMBER_FREQ_MULT)]
    return root * mult
...
def make_beep_pcm(freq: int, duration: float, volume: float = 1.0,
    ...
    """
    Generate a tone with `pulses` repetitions separated by `gap_ms` silence.
    Envelope: snappy attack, flat sustain, short decay — punchy and present.
    """
    def one_pulse(f, dur, vol):
        n      = int(TARGET_RATE * dur)
        t      = np.arange(n) / TARGET_RATE
        att    = int(n * 0.04)    # 4% — near-instant snap
        sus    = int(n * 0.75)    # 75% flat sustain
        dec    = n - att - sus
        env    = np.concatenate([
            np.linspace(0.0, 1.0, att),
            np.ones(sus),
            np.linspace(1.0, 0.0, dec),
        ])
        wave   = np.sin(2 * math.pi * f * t)
        wave  += 0.25 * np.sin(2 * math.pi * f * 2 * t)  # brighter 2nd harmonic
        wave  /= 1.25
...
def _ensure_group_tones(gid: int, vid: int, grp_idx: int, num_members: int):
    """Pre-generate tones for every member slot in a group if not already cached."""
    for m_idx in range(num_members):
        tk = _tone_key(gid, vid, grp_idx, m_idx)
        if tk not in pcm_cache:
            freq   = _tone_freq(grp_idx, m_idx)
            # Lead gets 2 pulses (clear "go"), joiners get 1 pulse
            pulses = 2 if m_idx == 0 else 1
            store(tk, make_beep_pcm(freq, 0.28, pulses=pulses))
            print(f"  tone {tk}: {freq:.0f}Hz x{pulses} → {clip_dur(tk):.3f}s")
...
class AddMemberModal(discord.ui.Modal, title="Add Member"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_data()
        await _panel_refresh(self.gid, self.vid, groups, interaction.guild, self._bots)
        summary = " + ".join(f"**{n}** ({t}s)" + (" *(lead)*" if i == 0 else "")
...
class RallyStartView(discord.ui.View):
    ...
    def _make_cb(self, group_idx, instant):
        async def cb(interaction: discord.Interaction):
            ...
            # Lead always gets their unique group tone (2 pulses = "go")
            ...
            if instant:
                ...
                await interaction.followup.send(
                    f"⚡ Instant: Group {group_idx + 1} ({members[0][0]})", ephemeral=True)
            else:
                ...
                await asyncio.gather(*coros, run_member_countdowns(vc, members, t0, cross))
                await interaction.followup.send(
                    f"Started Group {group_idx + 1}: {members[0][0]} (10s cooldown)",
                    ephemeral=True)
        return cb
